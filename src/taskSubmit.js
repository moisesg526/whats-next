export function taskSubmit(e, form, display) {
    e.preventDefault(); // Prevent page reload on submit
  
    const formData = new FormData(form);
    const title = form.querySelector("input[name='title']");
    const details = form.querySelector("input[name='details']");
    const priorityInput = form.querySelector("input[name='options']:checked");
    ...
    import("./projects").then((module) => {
      ...
      let displayPriority = document.createElement("p");
      displayPriority.textContent = priorityInput
        ? priorityInput.value
        : "No priority selected"; // Display selected priority
      projectCard.appendChild(displayPriority);
